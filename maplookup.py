...
IV_rootfolder = r"C:\Users\ag\HugeData\Greenland IV"
...
def load_vxvy(lat, lon, w, source="dtu", smoothing_sigma=None):
    source = source.lower()
    if "measures" in source:
        ...
        fvx = f"{IV_rootfolder}/MEaSUREs/M Multi-year IV mosaic v1/greenland_vel_mosaic250_vx_v1.tif"
    elif "dtu" in source:
        fvx = f"{IV_rootfolder}/DTU-Space/greenland_iv_50m_s1_20191216_20200125_zwally21_winter_v1.0_vx.tif"
    elif "tsx" in source:
        fvx = f"{IV_rootfolder}/TSX/TerraSAR-X_vel_AWI_V0.2_vx.tif"
    ...
    vx = vx.rio.clip_box(minx=np.min(x0) - w, miny=np.min(y0) - w, maxx=np.max(x0) + w, maxy=np.max(y0) + w)
    vy = rio.open_rasterio(fvy, band_as_variable=True).band_1
    vy = vy.rio.clip_box(minx=np.min(x0) - w, miny=np.min(y0) - w, maxx=np.max(x0) + w, maxy=np.max(y0) + w)
    ...
    if smoothing_sigma is not None:
        dx = np.abs(vx.x[1] - vx.x[0]).values
        vx = vx.copy(data=gaussian_filter(vx.data, smoothing_sigma / dx))
        vy = vy.copy(data=gaussian_filter(vy.data, smoothing_sigma / dx))
...
def get_strainrate(lat, lon, source="dtu", return_eigen_strainrate=False, smoothing_sigma=None):
    w = 1000 + smoothing_sigma * 2 if smoothing_sigma else 1000  # only work on a small window.
    vx, vy = load_vxvy(lat, lon, w, source, smoothing_sigma=smoothing_sigma)
